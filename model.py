...
class ABAE(torch.nn.Module):
    def __init__(self, word_num, aspects_num=14, emb_seize=200):
        super(ABAE, self).__init__()
        self.emb_seize = emb_seize
        self.word_num = word_num
        self.aspects_num = aspects_num
        self.w_embs = torch.nn.Embedding(word_num, emb_seize)
        self.a_embs = torch.randn(aspects_num, emb_seize, requires_grad=True)
        self.attention_M = torch.randn(emb_seize, emb_seize, requires_grad=True)
        self.sentence_aspects = torch.nn.Linear(emb_seize, aspects_num)
    ...
    def forward(self, ps, ns_list):
        ...
        return rs, [self.make_sentence_raw(self.w_embs(ns)) for ns in ns_list]